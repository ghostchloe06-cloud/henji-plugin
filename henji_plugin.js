(function() {
  ...
  /* ─── SVG 图标库 ─── */
  var ICONS = {
    back: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>',
    plus: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="14" y1="6" x2="14" y2="22"/><line x1="6" y1="14" x2="22" y2="14"/></svg>',
  ...
  function buildParallelSystemPrompt(session) {
    ...
    return buildCharacterProfile(char).then(function(baseProfile) {
      ...
      lines.push("4. 每次输出接下来的 3~10 轮（文字与动作都算一轮），具体轮数由当前情境的张力决定：平淡的日常交流可以少一些，情绪爆发、冲突或重要发现的场景可以多一些。speaker 字段必须是分身的 speakerId（如 self_1），不能是" + userName + "。");
      lines.push("5. 不要停留在寒暄或反复试探——要主动推进剧情：让分身们提出问题、做出反应、采取行动、发现新的细节或产生新的情绪转折，使每一轮生成都让局面往前走一步，而不是原地打转。");
      lines.push("6. 必须只输出合法 JSON，不要输出任何 JSON 之外的文字：");
  ...
  function renderChatMessage(session, msg) {
    ...
    var displayText = stripTtsMarkup(msg.text);
  ...
  function getStyles() {
    return [
      ...
      "." + ROOT_CLASS + " .hj-regen-banner{display:flex;align-items:center;gap:8px;padding:10px 20px;color:#6e6e73;font-size:12px;background:#f7f7f8;border-bottom:1px solid #ececee}",
      ...
      "." + ROOT_CLASS + " .hj-detail-p{font-size:16px;line-height:1.9;color:#1c1c1e;margin-bottom:16px}",
      ...
      "." + ROOT_CLASS + " .hj-msg-time{font-size:11px;color:#9a9a9e;margin-top:4px;margin-left:2px}",
      "." + ROOT_CLASS + " .hj-msg-row.me .hj-msg-time{margin-left:0;margin-right:2px}",
      "." + ROOT_CLASS + " .hj-msg-away-tag{margin-left:6px;padding:1px 6px;border-radius:8px;background:#f2f2f7;color:#9a9a9e;font-size:11px}",
      ...
      "." + ROOT_CLASS + " .hj-msg-action{text-align:center;font-size:12px;font-style:italic;color:#9a9a9e;margin:14px 30px}",
      ...
      "." + ROOT_CLASS + " .hj-pending-item{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:7px 10px;border-radius:10px;background:#f2f2f7;font-size:12px;color:#1c1c1e}",
      ...
      "." + ROOT_CLASS + " .hj-chat-toggle{width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#6e6e73;flex-shrink:0;cursor:pointer}",
      ...
      "." + ROOT_CLASS + " .hj-chat-send-btn{width:32px;height:32px;border-radius:50%;background:#1c1c1e;color:#ffffff;display:flex;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer}",

(function() {
  ...
  var EMOTION_COLORS = {
    "温柔记忆": { bg: "rgba(232,160,191,0.16)", fg: "#e8a0bf", dot: "#e8a0bf" },
    "创伤记忆": { bg: "rgba(224,90,90,0.16)", fg: "#e0807a", dot: "#e0807a" },
    "普通记忆": { bg: "rgba(255,255,255,0.08)", fg: "var(--text-secondary, #9a9a9a)", dot: "var(--text-hint, #6b6b6b)" },
    "高光记忆": { bg: "rgba(230,190,90,0.16)", fg: "#e6c05a", dot: "#e6c05a" }
  };
  ...
  function buildArticlePrompt(char, entry, context) {
    return buildCharacterProfile(char).then(function(profile) {
      var sys = "你是一位细腻的角色内心独白作者。请基于给定的标题与关键词，把这段记忆写成一篇第一人称（角色本人视角）的短篇记忆片段。\n" +
        "要求：\n" +
        "1. 篇幅 200~450 字，分 2~4 段，段落之间用换行分隔。\n" +
        "2. 文风贴合给出的'情绪底色'与'记忆类型'，克制、有画面感，避免说教式总结。\n" +
        "3. 可以包含内心独白、感官细节、简短的对话片段。\n" +
        "4. 严格贴合角色设定与关键词，不要引入无关的新设定。\n" +
        "5. 直接输出正文，不要标题、不要任何解释或前后缀、不要使用 markdown。";
  ...
  function avatarHtml(url, fallbackText, size) {
    ...
    return '<div style="width:' + size + 'px;height:' + size + 'px;border-radius:50%;background:var(--bg-secondary,#1a1a1a);display:flex;align-items:center;justify-content:center;font-size:' + Math.round(size * 0.4) + 'px;color:var(--text-secondary,#9a9a9a);font-weight:600;flex-shrink:0">' + escapeHtml(fb) + "</div>";
  }
  ...
  function getStyles() {
    return [
      ...
      "." + ROOT_CLASS + "{display:flex;flex-direction:column;height:100%;background:var(--bg-primary,#000);color:var(--text-primary,#f5f5f5);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}",
      "." + ROOT_CLASS + " .hj-header{display:flex;align-items:center;padding:10px 12px;padding-top:calc(10px + var(--safe-top,0px));border-bottom:1px solid var(--bg-secondary,#161616);position:sticky;top:0;z-index:100;min-height:52px;flex-shrink:0;background:var(--bg-primary,#000)}",
      ...
      "." + ROOT_CLASS + " .hj-header-subtitle{font-size:11px;letter-spacing:.08em;color:var(--text-hint,#6b6b6b);font-style:italic;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
      "." + ROOT_CLASS + " .hj-icon-btn{width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:50%;cursor:pointer;color:var(--text-primary,#f5f5f5);transition:background .2s,transform .1s}",
      "." + ROOT_CLASS + " .hj-icon-btn:active{transform:scale(.9);background:var(--bg-secondary,#161616)}",
      ...
      "." + ROOT_CLASS + " .hj-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:70px 24px;color:var(--text-hint,#6b6b6b);text-align:center;gap:10px}",
      ...
      "." + ROOT_CLASS + " .hj-list-item{display:flex;align-items:center;gap:12px;padding:14px 18px;cursor:pointer;border-bottom:1px solid var(--bg-secondary,#161616);transition:background .15s}",
      "." + ROOT_CLASS + " .hj-list-item:active{background:var(--bg-secondary,#161616)}",
      ...
      "." + ROOT_CLASS + " .hj-list-sub{font-size:13px;color:var(--text-secondary,#9a9a9a);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
      "." + ROOT_CLASS + " .hj-list-chevron{color:var(--text-hint,#6b6b6b);flex-shrink:0}",
      "." + ROOT_CLASS + " .hj-timeline-intro{padding:22px 20px 6px}",
      "." + ROOT_CLASS + " .hj-timeline-intro-title{font-size:20px;font-weight:700}",
      "." + ROOT_CLASS + " .hj-timeline-intro-sub{font-size:11px;letter-spacing:.1em;color:var(--text-hint,#6b6b6b);font-style:italic;margin-top:2px;text-transform:uppercase}",
      "." + ROOT_CLASS + " .hj-timeline-intro-desc{font-size:13px;color:var(--text-secondary,#9a9a9a);margin-top:10px;line-height:1.6}",
      "." + ROOT_CLASS + " .hj-actions-row{display:flex;gap:10px;padding:16px 20px 4px}",
      "." + ROOT_CLASS + " .hj-btn{display:flex;align-items:center;justify-content:center;gap:6px;padding:11px 16px;border-radius:var(--radius-md,14px);font-size:14px;font-weight:600;cursor:pointer;border:none;transition:transform .1s,opacity .2s}",
      "." + ROOT_CLASS + " .hj-btn:active{transform:scale(.97)}",
      "." + ROOT_CLASS + " .hj-btn[disabled]{opacity:.45;pointer-events:none}",
      "." + ROOT_CLASS + " .hj-btn svg{width:16px;height:16px;flex-shrink:0}",
      "." + ROOT_CLASS + " .hj-btn-primary{background:var(--text-primary,#f5f5f5);color:var(--bg-primary,#000)}",
      "." + ROOT_CLASS + " .hj-btn-outline{background:transparent;border:1.5px solid var(--border-color,rgba(255,255,255,.16));color:var(--text-primary,#f5f5f5)}",
      ...
      "." + ROOT_CLASS + " .hj-timeline-rail::before{content:'';flex:1;width:1.5px;background:var(--bg-secondary,#222)}",
      ...
      "." + ROOT_CLASS + " .hj-timeline-card{flex:1;background:var(--bg-card,#111);border:1px solid var(--border-color,rgba(255,255,255,.08));border-radius:var(--radius-lg,18px);padding:16px;margin-bottom:16px;cursor:pointer;transition:transform .15s}",
      ...
      "." + ROOT_CLASS + " .hj-age-label{font-size:11px;font-style:italic;letter-spacing:.06em;color:var(--text-hint,#6b6b6b);text-transform:uppercase}",
      ...
      "." + ROOT_CLASS + " .hj-card-excerpt{font-size:13px;font-style:italic;color:var(--text-secondary,#9a9a9a);line-height:1.6;margin-bottom:10px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}",
      "." + ROOT_CLASS + " .hj-card-tags{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px}",
      "." + ROOT_CLASS + " .hj-tag-pill{font-size:11px;padding:3px 9px;border-radius:10px;background:var(--bg-secondary,#1a1a1a);color:var(--text-secondary,#9a9a9a)}",
      "." + ROOT_CLASS + " .hj-card-tone{font-size:12px;color:var(--text-hint,#6b6b6b)}",
      "." + ROOT_CLASS + " .hj-card-pending{font-size:11px;color:var(--text-hint,#6b6b6b);margin-top:8px;text-align:right}",
      "." + ROOT_CLASS + " .hj-add-placeholder{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:48px 20px;border:1.5px dashed var(--border-color,rgba(255,255,255,.16));border-radius:var(--radius-lg,18px);color:var(--text-hint,#6b6b6b);cursor:pointer}",
      "." + ROOT_CLASS + " .hj-add-entry-row{display:flex;align-items:center;justify-content:center;gap:8px;padding:14px;border:1.5px dashed var(--border-color,rgba(255,255,255,.16));border-radius:var(--radius-lg,18px);color:var(--text-secondary,#9a9a9a);cursor:pointer;font-size:13px;margin-left:22px}",
      "." + ROOT_CLASS + " .hj-portal-card{display:flex;align-items:center;gap:14px;margin:24px 20px;padding:18px;border-radius:var(--radius-xl,22px);background:var(--bg-card,#111);border:1px solid var(--border-color,rgba(255,255,255,.1));cursor:pointer}",
      "." + ROOT_CLASS + " .hj-portal-card:active{transform:scale(.98)}",
      "." + ROOT_CLASS + " .hj-portal-icon{width:44px;height:44px;border-radius:50%;background:var(--bg-secondary,#1a1a1a);display:flex;align-items:center;justify-content:center;flex-shrink:0}",
      "." + ROOT_CLASS + " .hj-portal-text{flex:1;min-width:0}",
      "." + ROOT_CLASS + " .hj-portal-title{font-size:16px;font-weight:700}",
      "." + ROOT_CLASS + " .hj-portal-sub{font-size:12px;color:var(--text-secondary,#9a9a9a);margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
      "." + ROOT_CLASS + " .hj-portal-arrow{color:var(--text-hint,#6b6b6b);flex-shrink:0}",
      "." + ROOT_CLASS + " .hj-modal-overlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.55);z-index:400;display:flex;align-items:center;justify-content:center;padding:24px;animation:hjFadeIn .18s}",
      "." + ROOT_CLASS + " .hj-modal-card{width:100%;max-width:340px;background:var(--bg-card,#161616);border-radius:var(--radius-xl,22px);padding:22px;animation:hjPopIn .2s}",
      ...
      "." + ROOT_CLASS + " .hj-form-label{font-size:12px;color:var(--text-secondary,#9a9a9a);margin-bottom:6px}",
      "." + ROOT_CLASS + " .hj-input{width:100%;padding:11px 14px;border-radius:var(--radius-md,14px);border:1px solid var(--border-color,rgba(255,255,255,.14));background:var(--bg-primary,#000);color:var(--text-primary,#f5f5f5);font-size:14px;outline:none}",
      "." + ROOT_CLASS + " .hj-input:focus{border-color:var(--text-primary,#f5f5f5)}",
      "." + ROOT_CLASS + " .hj-modal-actions{display:flex;gap:10px;margin-top:6px}",
      "." + ROOT_CLASS + " .hj-loading-inline{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;padding:60px 20px;color:var(--text-secondary,#9a9a9a);font-size:13px}",
      "." + ROOT_CLASS + " .hj-spinner{width:26px;height:26px;border-radius:50%;border:2.5px solid var(--bg-secondary,#222);border-top-color:var(--text-primary,#f5f5f5);animation:hjSpin .8s linear infinite}",
      "." + ROOT_CLASS + " .hj-spinner-sm{width:14px;height:14px;border-radius:50%;border:2px solid var(--bg-secondary,#222);border-top-color:var(--text-secondary,#9a9a9a);animation:hjSpin .8s linear infinite;flex-shrink:0}",
      ...
      "." + ROOT_CLASS + " .hj-detail-p{font-size:15px;line-height:1.9;color:var(--text-primary,#f0f0f0);margin-bottom:16px}",
      "." + ROOT_CLASS + " .hj-setup-hint{font-size:13px;color:var(--text-secondary,#9a9a9a);padding:18px 20px 10px}",
      "." + ROOT_CLASS + " .hj-select-list{padding:0 20px;display:flex;flex-direction:column;gap:10px}",
      "." + ROOT_CLASS + " .hj-select-card{display:flex;align-items:center;gap:12px;padding:14px;border-radius:var(--radius-md,14px);border:1.5px solid var(--border-color,rgba(255,255,255,.1));cursor:pointer;transition:border-color .15s,background .15s}",
      "." + ROOT_CLASS + " .hj-select-card.selected{border-color:var(--text-primary,#f5f5f5);background:var(--bg-secondary,#1a1a1a)}",
      "." + ROOT_CLASS + " .hj-select-check{width:20px;height:20px;border-radius:50%;border:1.5px solid var(--border-color,rgba(255,255,255,.2));display:flex;align-items:center;justify-content:center;flex-shrink:0;color:var(--bg-primary,#000)}",
      "." + ROOT_CLASS + " .hj-select-card.selected .hj-select-check{background:var(--text-primary,#f5f5f5);border-color:var(--text-primary,#f5f5f5)}",
      "." + ROOT_CLASS + " .hj-select-age{font-size:11px;color:var(--text-hint,#6b6b6b);font-style:italic;text-transform:uppercase}",
      "." + ROOT_CLASS + " .hj-select-title{font-size:14px;font-weight:600;margin-top:2px}",
      "." + ROOT_CLASS + " .hj-segmented{display:flex;margin:0 20px;border-radius:var(--radius-md,14px);overflow:hidden;border:1px solid var(--border-color,rgba(255,255,255,.14))}",
      "." + ROOT_CLASS + " .hj-segmented-btn{flex:1;text-align:center;padding:11px;font-size:13px;cursor:pointer;color:var(--text-secondary,#9a9a9a)}",
      "." + ROOT_CLASS + " .hj-segmented-btn.active{background:var(--text-primary,#f5f5f5);color:var(--bg-primary,#000);font-weight:600}",
      ...
      "." + ROOT_CLASS + " .hj-msg-name{font-size:11px;color:var(--text-hint,#6b6b6b);margin-bottom:4px;margin-left:2px}",
      "." + ROOT_CLASS + " .hj-msg-bubble{background:var(--bg-card,#181818);border:1px solid var(--border-color,rgba(255,255,255,.08));color:var(--text-primary,#f0f0f0);padding:10px 14px;border-radius:16px 16px 16px 4px;font-size:14px;line-height:1.55;white-space:pre-wrap;word-break:break-word}",
      "." + ROOT_CLASS + " .hj-msg-bubble.me{background:var(--text-primary,#f5f5f5);color:var(--bg-primary,#000);border-radius:16px 16px 4px 16px;border:none}",
      "." + ROOT_CLASS + " .hj-msg-time{font-size:10px;color:var(--text-hint,#6b6b6b);margin-top:4px;margin-left:2px}",
      "." + ROOT_CLASS + " .hj-msg-row.me .hj-msg-time{margin-left:0;margin-right:2px}",
      "." + ROOT_CLASS + " .hj-msg-action{text-align:center;font-size:12.5px;font-style:italic;color:var(--text-hint,#6b6b6b);margin:14px 30px}",
      "." + ROOT_CLASS + " .hj-chat-typing{display:flex;align-items:center;gap:8px;padding:6px 4px;color:var(--text-hint,#6b6b6b);font-size:12px}",
      "." + ROOT_CLASS + " .hj-chat-footer{border-top:1px solid var(--bg-secondary,#161616);padding:10px 14px;padding-bottom:calc(10px + var(--safe-bottom,0px));display:flex;flex-direction:column;gap:8px;background:var(--bg-primary,#000)}",
      "." + ROOT_CLASS + " .hj-chat-input-row{display:flex;align-items:center;gap:8px}",
      "." + ROOT_CLASS + " .hj-chat-toggle{width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:var(--text-secondary,#9a9a9a);flex-shrink:0;cursor:pointer}",
      "." + ROOT_CLASS + " .hj-chat-toggle.active{background:var(--text-primary,#f5f5f5);color:var(--bg-primary,#000)}",
      "." + ROOT_CLASS + " .hj-chat-input{flex:1;padding:10px 14px;border-radius:20px;border:1px solid var(--border-color,rgba(255,255,255,.14));background:var(--bg-card,#141414);color:var(--text-primary,#f5f5f5);font-size:14px;outline:none;min-width:0}",
      "." + ROOT_CLASS + " .hj-chat-send-btn{width:34px;height:34px;border-radius:50%;background:var(--text-primary,#f5f5f5);color:var(--bg-primary,#000);display:flex;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer}",
